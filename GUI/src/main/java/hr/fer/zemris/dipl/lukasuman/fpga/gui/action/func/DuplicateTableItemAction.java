...
public class DuplicateTableItemAction<T extends Duplicateable & Nameable> extends AbstractTableItemAction<T> {
    ...
    public DuplicateTableItemAction(JFPGA jfpga, Supplier<AbstractGUIController<T>> controllerProvider,
                                    String localizationKey, String targetLocalizationKey) {
        super(jfpga, controllerProvider, localizationKey, Utility.checkNull(targetLocalizationKey, "localization key"));
    }
    ...
    protected void doAction(AbstractGUIController<T> controller) {
        if (controller.getIndexSelectedItem() < 0) {
            ...
            jfpga.showWarningMsg(String.format("%s " + locProv.getString(LocalizationKeys.SELECT_S_FROM_THE_TABLE_MSG_KEY),
                    locProv.getString(LocalizationKeys.NOTHING_TO_DUPLICATE_MSG_KEY),
